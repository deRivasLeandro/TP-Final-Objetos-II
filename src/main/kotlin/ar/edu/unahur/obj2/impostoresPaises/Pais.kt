...
class Pais(
    ...
    var bloquesRegionales: MutableSet<String>,
    var idiomasOficiales: MutableSet<String>
) {
    ...
    val paisesLimitrofes: MutableSet<Pais> = mutableSetOf()

    fun esPlurinacional(): Boolean {
        return idiomasOficiales.size >= 2
    }
    ...
    fun esLimitrofeDe(pais: Pais): Boolean {
        if (pais == this) {
            error("Un país no puede ser limítrofe de sí mismo.")
        }
    ...
    fun pertenecenAUnMismoBloque(pais: Pais): Boolean {
        return bloquesRegionales.intersect(pais.bloquesRegionales).isNotEmpty()
    }

    fun esPotencialAliadoDe(pais: Pais): Boolean {
        return !necesitaTraductorCon(pais) && pertenecenAUnMismoBloque(pais)
    }

    fun convieneIrDeComprasA(pais: Pais): Boolean {
        if (pais == this) {
            error("La consulta debe hacerse entre dos países diferentes.")
        }
        return cotizacionDolar < pais.cotizacionDolar
    }

    fun aCuantoEquivale(pais: Pais, monto: Double): Double {
        return monto / cotizacionDolar * pais.cotizacionDolar
    }

    fun agregarPaisesLimitrofes(paises: MutableSet<Pais>) {
        paisesLimitrofes.addAll(paises)
    }

    fun sacarPaises(paises: MutableSet<Pais>) {
        paisesLimitrofes.removeAll(paises)
    }
